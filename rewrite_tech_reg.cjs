...
export default function TechnicianRegistration() {
  ...
  const handleSendOtp = () => {
    if (formData.mobile.length >= 10) {
      ...
      setOtpSent(true);
      alert(\`Your verification OTP is: \${newOtp}\`);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const application: TechnicianApplication = {
      ...
      await addDoc(collection(db, 'technicianApplications'), application);
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-1 md:col-span-2 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    {!documents.photo && !isCameraActive && (
                      <div className="flex gap-4">
                        <button type="button" onClick={startCamera} className="flex-1 flex flex-col items-center justify-center gap-2 p-6 rounded-xl border-2 border-dashed border-blue-300 bg-blue-50 hover:bg-blue-100 transition-colors text-blue-700 font-semibold">
                        ...
                        <label className="flex-1 flex flex-col items-center justify-center gap-2 p-6 rounded-xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 transition-colors text-slate-600 font-semibold cursor-pointer">
                          ...
                          <input type="file" accept="image/*,application/pdf" className="hidden" onChange={(e) => handleFileUpload(e, 'photo')} />
                        </label>
                      </div>
                    )}
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    {documents.aadhaarFront ? (
                      ...
                      <label className="flex flex-col items-center justify-center gap-2 h-32 rounded-xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 cursor-pointer transition-colors text-sm font-medium text-slate-500">
                        <Upload className="w-6 h-6 text-slate-400" /> Upload Front Side
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    {documents.aadhaarBack ? (
                      ...
                      <label className="flex flex-col items-center justify-center gap-2 h-32 rounded-xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 cursor-pointer transition-colors text-sm font-medium text-slate-500">
                        <Upload className="w-6 h-6 text-slate-400" /> Upload Back Side
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    {documents.pan ? (
                      ...
                      <label className="flex flex-col items-center justify-center gap-2 h-32 rounded-xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 cursor-pointer transition-colors text-sm font-medium text-slate-500">
                        <Upload className="w-6 h-6 text-slate-400" /> Upload PAN
            ...
            <div className="mt-10 pt-6 border-t border-slate-100 flex justify-between items-center">
              ...
              <button 
                type="submit" 
                disabled={isSubmitting || (step === 1 && (!otpVerified || !formData.fullName || !formData.city || !formData.mobile))}
