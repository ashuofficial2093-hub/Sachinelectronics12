...
export default function AdminAnalytics({ complaints, technicians }: AdminAnalyticsProps) {
  ...
  const performanceData = useMemo(() => {
    ...
    complaints.forEach(complaint => {
      if (complaint.status === 'Resolved' && complaint.totalCost) {
        const cost = parseFloat(complaint.totalCost.replace(/[^0-9.-]+/g, '')) || 0;
        const profit = cost * 0.45; // Simulated profit margin
        const cDate = new Date(complaint.updatedAt || complaint.createdAt);
  ...
  const totalRevenue = useMemo(() => performanceData.reduce((acc, curr) => acc + curr.revenue, 0), [performanceData]);
  const totalRepairs = useMemo(() => performanceData.reduce((acc, curr) => acc + curr.repairs, 0), [performanceData]);
  const totalProfit = useMemo(() => performanceData.reduce((acc, curr) => acc + curr.profit, 0), [performanceData]);
